...
float4 TexCoordsWorld(VertexInput v)
{
	float4 posWorld = mul(unity_ObjectToWorld, v.vertex);
	float4 texcoord;

	if(abs(v.normal.x) > 0.5)
	{
		texcoord.xy = TRANSFORM_TEX(posWorld.yz, _MainTex);
		texcoord.zw = TRANSFORM_TEX(posWorld.yz, _DetailAlbedoMap);
	}
	else if(abs(v.normal.z) > 0.5)
	{
		texcoord.xy = TRANSFORM_TEX(posWorld.xy, _MainTex);
		texcoord.zw = TRANSFORM_TEX(posWorld.xy, _DetailAlbedoMap);
	}
	else 
	{
		texcoord.xy = TRANSFORM_TEX(posWorld.xz, _MainTex);
		texcoord.zw = TRANSFORM_TEX(posWorld.xz, _DetailAlbedoMap);
	}
